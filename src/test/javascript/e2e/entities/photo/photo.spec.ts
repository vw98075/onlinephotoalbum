...
describe('Photo e2e test', () => {
  ...
  describe('Create flow', () => {
    it('should load create Photo page', async () => {
      ...
      expect(await updatePage.title.getAttribute('id')).to.match(/onlinephotoalbumApp.photo.home.createOrEditLabel/);
    });
    ...
    it('should create and save Photos', async () => {
      ...
      await updatePage.takenInput.sendKeys('01/01/2001' + protractor.Key.TAB + '02:30AM');
      expect(await updatePage.takenInput.getAttribute('value')).to.contain('2001-01-01T02:30');

      await updatePage.uploadedInput.sendKeys('01/01/2001' + protractor.Key.TAB + '02:30AM');
      expect(await updatePage.uploadedInput.getAttribute('value')).to.contain('2001-01-01T02:30');

      // await  selectLastOption(updatePage.albumSelect);
    ...
    describe('Details, Update, Delete flow', () => {
      after(async () => {
        ...
        expect(await deleteDialog.title.getAttribute('id')).to.match(/onlinephotoalbumApp.photo.delete.question/);
      ...
      it('should load edit Photo page, fetch data and update', async () => {
        ...
        await updatePage.takenInput.clear();
        await updatePage.takenInput.sendKeys('01/01/2019' + protractor.Key.TAB + '02:30AM');
        expect(await updatePage.takenInput.getAttribute('value')).to.contain('2019-01-01T02:30');

        await updatePage.uploadedInput.clear();
        await updatePage.uploadedInput.sendKeys('01/01/2019' + protractor.Key.TAB + '02:30AM');
        expect(await updatePage.uploadedInput.getAttribute('value')).to.contain('2019-01-01T02:30');
